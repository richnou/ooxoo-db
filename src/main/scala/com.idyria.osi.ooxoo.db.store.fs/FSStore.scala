...
class FSContainer(
  ...
  def documents : Iterable[Document] = {
    ...
    this.baseFolder.listFiles() match {
      case null =>  Nil
      case files => files.filter(_.isFile()).map(f => this.document(f.getName).get)
    }
  ...
  def document(path: String): Option[Document] = {
    ...
    //-- Create File
    var documentFile = new File(baseFolder, path)
  ...
  def writeDocument[T <: Buffer](path: String, topElement: T): Unit  = {
	  ...
	  //-- Get Document
	  var document = this.document(path) match {
	    case Some(doc) => doc
	      
	    //-- Create   
	    case None =>
	      
	      var doc = new FSDocument(new File(baseFolder,path))
	      doc
	  }
